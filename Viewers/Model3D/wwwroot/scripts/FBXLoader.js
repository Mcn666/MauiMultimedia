	...
	class FBXTreeParser {
		...
		parseImages() {
			...
			for ( const id in images ) {
				...
				if ( blobs[ filename ] !== undefined ) images[ id ] = blobs[ filename ]; else {

					// Keep relative subdirectories but strip absolute path prefixes.
					// Original code used .split('\\').pop() which lost "textures/" subdirectory.
					let p = images[ id ].replace( /\\/g, '/' );  // normalize backslashes
					// Strip drive letter prefix: "C:/..." → "/..."
					const ci = p.indexOf( ':' );
					if ( ci > 0 && ci < 4 ) p = p.substring( ci + 1 );
					// Strip leading slash from Unix/Windows absolute paths
					p = p.replace( /^\//, '' );
					images[ id ] = p;
		...
		parseImage( videoNode ) {
			...
			case 'dds':
				// DDS can't be decoded by <img>. Check if we have a pre-converted
				// PNG data URI (from C# ImageProcessingService.DecodeDds).
				if (typeof window.__textureData !== 'undefined') {
					var texFileName = fileName.split('\\').pop().split('/').pop();
					if (window.__textureData[texFileName]) {
						return window.__textureData[texFileName];
					}
				}
		...
		loadTexture( textureNode, images ) {
			...
			if ( children !== undefined && children.length > 0 && images[ children[ 0 ].ID ] !== undefined ) {
				...
				if ( fileName.indexOf( 'data:' ) === 0 ) {
					...
					// Data URI (pre-converted PNG): create texture directly,
					// bypassing TextureLoader which may convert it to a blob URL.
					// DDS data was already flipped in C# (DecodeDds → FlipVertical),
					// so we need flipY=false to prevent Three.js from re-flipping it.
