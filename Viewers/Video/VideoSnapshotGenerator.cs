...
/// 这两部分与图片快照共享基类 <see cref="SnapshotServiceBase"/>，方法名刻意区分
/// （generateVideoSnapshot）以避免同名 JSInvokable 互相覆盖。
/// </summary>
public static class VideoSnapshotGenerator
{
    private static readonly VideoSnapshotService _instance = new();
    ...
    public static Task<string?> GenerateVideoSnapshot(string filePath)
        => _instance.GetSnapshotAsync(filePath);

    private sealed class VideoSnapshotService : SnapshotServiceBase
    {
        // 平台原生取帧实现注册点：由 EnsureExtractorRegistered 从已加载程序集发现并填充。
        // 未注册（不支持的平台 / 未发现实现）时为 null，调用方回退 🎬 图标。
        public Func<string, Task<byte[]?>>? Extractor { get; set; }

        // 注册发现仅执行一次
        private readonly object _regLock = new();
        private bool _regSearched;

        protected override async Task<string?> GenerateAsync(string filePath)
        {
        ...
        /// </summary>
        private void EnsureExtractorRegistered()
        {
        ...
        private bool TryFindAndRegister(Assembly asm)
        {
            try
            {
